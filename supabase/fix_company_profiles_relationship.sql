-- =====================================================================
-- Fix Foreign Key Relationship: company_info -> profiles
-- =====================================================================
-- This script establishes the proper foreign key relationship between
-- company_info.owner_id and profiles.id to enable Supabase joins.
--
-- Safe to run multiple times (idempotent)
-- =====================================================================

-- Step 1: Verify that both tables exist
DO $$
BEGIN
...
-- Step 2: Add owner_id column to company_info if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Added owner_id column to company_info ✓';
    ELSE
        RAISE NOTICE 'owner_id column already exists ✓';
    END IF;
END $$;

-- Step 3: Clean up any invalid owner_id references
-- (Set to NULL any owner_ids that don't have a corresponding profile)
UPDATE company_info
SET owner_id = NULL
WHERE owner_id IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM profiles WHERE profiles.id = company_info.owner_id);

-- Step 4: Drop existing constraint if it exists (to recreate it properly)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 
        ...
        RAISE NOTICE 'Dropped existing constraint company_info_owner_id_fkey';
    END IF;
END $$;
...
-- Step 6: Create an index on owner_id for better join performance
